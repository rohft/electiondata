...
const buildTreeFromVoters = (
  ...
  // Build caste folders from categories + custom
  const allCastes = new Set([
    ...casteCategories.map((c) => c.name),
    ...customCastes,
    'Other',
  ]);
  ...
  return rootNodes.filter((n) => n.voterCount > 0 || customCastes.includes(n.id));
};
...
export const CasteSection = () => {
  ...
  const treeData = useMemo(() => {
    return buildTreeFromVoters(allVoters, CASTE_CATEGORIES, customCastes);
  }, [allVoters, customCastes]);
  ...
  const confirmAddSubfolder = () => {
    ...
    // For now, add to customCastes (top-level) – deep nesting would need state tree management
  ...
  const confirmRename = () => {
    ...
    // Top-level folders: update customCastes, or simply show toast as placeholder
