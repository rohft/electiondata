...
const buildTreeFromVoters = (
  ...
  allCastes.forEach((casteName) => {
    ...
    const addSubfolders = (parent: TreeNode, subfolders: string[]) => {
    ...
    addSubfolders(folderNode, hierarchy.subfolders);
...
const IconViewItem: React.FC<{
  ...
  const isFolder = node.type === 'folder';
  const iconSize = size === 'extra-large' ? 'h-16 w-16' : 'h-12 w-12';
  const containerSize = size === 'extra-large' ? 'w-32 h-36' : 'w-24 h-28';
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <div
          className={cn(
            containerSize,
            'flex flex-col items-center justify-center gap-2 p-2 rounded-lg cursor-pointer transition-all select-none',
            'hover:bg-muted/50',
        ...
        >
          {isFolder ? (
            <FolderClosed className={cn(iconSize, 'text-warning')} />
          ) : (
            <User className={cn(iconSize, 'text-muted-foreground')} />
          )}
          <div className="text-center w-full">
            <p className="text-xs font-medium truncate w-full" title={node.name}>
              {node.name.split(' (')[0]}
            </p>
...
export const CasteSection = () => {
  ...
  // Current folder contents for icon view navigation
  const currentFolderContents = useMemo(() => {
    if (!currentFolderId) return treeData;
    ...
    return folder?.children || treeData;
  ...
  const handleSelect = useCallback(
    (id: string, e: React.MouseEvent) => {
      if (e.shiftKey && lastSelectedId) {
        const list = viewMode === 'list' ? flatList : currentFolderContents;
        const startIdx = list.findIndex((n) => n.id === lastSelectedId);
        const endIdx = list.findIndex((n) => n.id === id);
        if (startIdx !== -1 && endIdx !== -1) {
          const range = list.slice(Math.min(startIdx, endIdx), Math.max(startIdx, endIdx) + 1);
    ...
    [flatList, currentFolderContents, lastSelectedId, viewMode]
  );

  // Icon view double click to navigate into folder
  const handleIconDoubleClick = (node: TreeNode) => {
    if (node.type === 'folder') {
  ...
  // Navigate back in icon view
  const handleNavigateBack = () => {
    if (!currentFolderId) return;
    const findParent = (nodes: TreeNode[], targetId: string): string | null => {
      for (const node of nodes) {
        for (const child of node.children) {
          if (child.id === targetId) return node.parentId === null ? null : node.id;
        }
    ...
    const parent = findParent(treeData, currentFolderId);
  ...
  // CRUD handlers
  const handleAddSubfolder = (parent: TreeNode | null) => {
  ...
  const confirmDelete = () => {
    ...
    if (dialogTarget.type === 'folder') {
      if (dialogTarget.parentId) {
        // It's a subfolder - need to find parent caste name
        const parentPath = dialogTarget.path.split('/');
        const rootCaste = parentPath[0];
        removeSubfolder(rootCaste, dialogTarget.name);
      } else {
  ...
  const handleMoveSelected = (targetFolder: TreeNode) => {
    const list = viewMode === 'list' ? flatList : currentFolderContents;
    const selected = list.filter((n) => selectedIds.has(n.id) && n.type === 'surname');
  ...
  // Import/Export (includes all subfolders)
  const handleExport = () => {
    ...
    a.href = url;
    a.download = 'ethnic-group-hierarchy.json';
    a.click();
    URL.revokeObjectURL(url);
    toast.success('Ethnic group hierarchy exported (includes all subfolders)');
  };
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        toast.success('Ethnic group hierarchy imported (includes all subfolders)');
      } catch (err) {
  ...
  const getCurrentFolderName = () => {
    if (!currentFolderId) return 'All Ethnic Groups';
    ...
    return findNode(treeData)?.name.split(' (')[0] || 'Unknown';
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="card-shadow border-border/50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base font-semibold">
            ...
            Ethnic Group Hierarchy Management
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="gap-2">
                    ...
                    {viewMode === 'large-icons' && <LayoutGrid className="h-4 w-4" />}
                    {viewMode === 'extra-large-icons' && <Grid3X3 className="h-4 w-4" />}
                    View
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  ...
                  <DropdownMenuItem onClick={() => setViewMode('large-icons')}>
                    <LayoutGrid className="h-4 w-4 mr-2" />
                    Large Icons
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setViewMode('extra-large-icons')}>
              ...
              <Button size="sm" onClick={() => handleAddSubfolder(null)} className="gap-2">
          ...
          {viewMode !== 'list' && currentFolderId && (
            <div className="flex items-center gap-2 mb-4 pb-3 border-b border-border">
              ...
              <span className="text-sm text-muted-foreground">
                Current: <span className="font-medium text-foreground">{getCurrentFolderName()}</span>
              </span>
            </div>
          )}
              ...
              <div className="pr-4 space-y-0.5">
                ...
                {filteredTree.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p>No ethnic groups found. Upload voter data or add an ethnic group folder.</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex flex-wrap gap-4">
                {(currentFolderId ? currentFolderContents : filteredTree).map((node) => (
                  <IconViewItem
                    key={node.id}
                    node={node}
                    size={viewMode === 'extra-large-icons' ? 'extra-large' : 'large'}
                ...
                {(currentFolderId ? currentFolderContents : filteredTree).length === 0 && (
      ...
      <Dialog open={showAddFolderDialog} onOpenChange={setShowAddFolderDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {dialogTarget ? `Add Subfolder to ${dialogTarget.name.split(' (')[0]}` : 'Add New Ethnic Group'}
            </DialogTitle>
          </DialogHeader>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {dialogTarget?.type === 'folder'
                ? `This will remove "${dialogTarget?.name.split(' (')[0]}" and all its contents. Voters will need to be reassigned.`
